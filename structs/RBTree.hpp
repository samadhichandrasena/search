...
private:
    ...
    void removeHelper(NodePtr node, T item)
    {
        ...
        if (z == TNULL) {
		  fatal("Couldn't find key in the tree with fhat %f \n", cursorValueFn(item));
            return;
        }
    ...
    void printHelper(NodePtr root_, string indent, bool last)
    {
        ...
        if (root_ != TNULL) {
            ...
            cout << root_->data << " fh" << cursorValueFn(root_->data)
				 << "(" << sColor << ")";
            ...
            while (dupNode != TNULL) {
                ...
                cout << dupNode->data << " fh" << cursorValueFn(dupNode->data)
					 << "(" << sColor << ")" << " ->";
...
public:
    ...
    void push(T item)
    {
        ...
        while (x != TNULL) {
            ...
            if (comp(node->data, x->data)) {
                ...
                if (node->data == x->data) {
				  fatal("insert an existing node with fhat %f \n", cursorValueFn(item));
                    return;
                }
                x   = x->dupNext;
                dup = true;
    ...
    T front() { return minimum(root)->data; }
