...
template <class D> struct MonoOnwardDSearch : public SearchAlgorithm<D> {
	...
	MonoOnwardDSearch(int argc, const char *argv[]) :
  ...
  Node *dedup(D &d, Node *n) {
	...
	void search(D &d, typename D::State &s0) {
		...
		while (!done && !SearchAlgorithm<D>::limit()) {
			...
			while(c < slot_n && c < width && !SearchAlgorithm<D>::limit()) {
 			  n = beam[c];
			  beam[c] = NULL;
			  c++;
			...
			while(i < slot_n && i < width && !open.empty()) {
			  n = open.pop();
			  n->width_seen = slot_n-1;
			  beam[i] = dedup(d, n);
			  if(beam[i]) {
